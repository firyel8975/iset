...
<div class="dropdown">
  ...
  <div class="dropdown-content">
	...
	<thead>
		<tr>
    <th><h1>Date</h1></th>
		<th><h1>Hardware</h1></th>
		<th><h1>Software</h1></th>
    <th><h1>Resultat finale</h1></th>
    <th><h1>explication</h1></th> 
    <th><h1></h1></th> 
    ...
    if ($_SESSION['mail'] !='') 
    {
      ...
      $stmt->execute();
                ...
                $sql = "SELECT datec, nom,hardware,software_exp,resultat_final,explication ,id  FROM formulaire_technicien  WHERE destinataire='$mail' AND monemail='$smail'";
                try {
                  $stmt = $dbh->query($sql);
                  $result = $stmt->setFetchMode(PDO::FETCH_NUM);
                  while ($row = $stmt->fetch()) {  
                      if($row[4]=='probleme resolu') 
                    print "<tr><td>".$row[0]."</td><td>" . $row[2]."</td><td> " .$row[3] . "</td><td><span class='v'>".$row[4] . "</span></td><td>".$row[5] . "</td><td><a class='button' href='reclamation2.php'><span class='ss'>Repondre</span></a> <td><input type='checkbox' id='scales' name='option[]' value='$row[6]'></td></tr>" ;
                   if($row[4]=='probleme non resolu')
                   print "<tr><td>".$row[0]."</td><td>" . $row[2]."</td><td> " .$row[3] . "</td><td><span >".$row[4] . "</span></td><td>".$row[5] . "</td><td><a class='button' href='reclamation2.php'><span class='ss'>Repondre</span></a></td><td><input type='checkbox' id='scales' name='option[]' value='$row[6]'></td></tr>" ;
                   else
                   print "<tr><td>".$row[1]."</td><td>".$row[0]."</td><td>" . $row[2]."</td><td> " .$row[3] . "</td><td><span class='v'>".$row[4] . "</span></td><td>".$row[5] . "</td><td><a class='button' href='reclamation2.php'><span class='ss'>Repondre</span></a> <td><input type='checkbox' id='scales' name='option[]' value='$row[6]'></td></tr>" ;
                  }
                }             
                ...
                foreach($_POST["option"] as $checkoptions){
                  $sql = "DELETE FROM formulaire_technicien WHERE id=$checkoptions ";
                   $dbh->exec($sql);
